...
crypto_type="ssl"  # Default crypto type (ssl, gpg, or aes)
...
set_crypto_type() {
    local new_type="$1"
    if [[ "$new_type" != "ssl" && "$new_type" != "gpg" && "$new_type" != "aes" ]]; then
        echo "Invalid crypto type. Use 'ssl', 'gpg', or 'aes'"
        return 1
    fi
...
setPass() {
    local char
    local input_pass=""
    local purpose="$1"  # "hash", "ssl", "gpg", or "aes"
...
encFile01() {
    ...
    for file in "${tree[@]}"; do
        ...
        if [[ "$file" == *.ssl ]] || [[ "$file" == *.gpg ]] || [[ "$file" == *.aes ]]; then
            # Strip the extension
            if [[ "$file" == *.ssl ]]; then
                ext="ssl"
            elif [[ "$file" == *.gpg ]]; then
...
case "$1" in
    ...
    settype)
        if [[ -z $2 ]]; then
            ...
            echo "Usage: $0 settype [ssl|gpg|aes]"
        else
            set_crypto_type "$2"
        fi
        ;;
    ...
    help|--help|-h)
        ...
        echo "  settype     - Set crypto type: $0 settype [ssl|gpg|aes] (default: ssl)"
