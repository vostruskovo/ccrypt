...
cpt="aes"        # Encryption method: "ssl", "gpg", or "aes" (default: aes)
...
case "$1" in
    ...
    decmd5)
        ...
        ;;
    setmode)
        if [[ "$2" == "ssl" || "$2" == "gpg" || "$2" == "aes" ]]; then
            cpt="$2"
            echo "Encryption mode set to: $cpt"
        else
            echo "Usage: $0 setmode [ssl|gpg|aes]"
        fi
        ;;
    setaes)
        set_aes_params
    ...
    help|--help|-h)
        ...
        echo ""
        echo "CONFIGURATION:"
        echo "  setmode  - Set encryption mode: ssl, gpg, or aes (current: $cpt)"
